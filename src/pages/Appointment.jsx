...
const Appointment = () => {
  ...
  const bookAppointment = async () => {
    ...
    if (!currentUser) return alert('Please log in to book an appointment.');

    const slotDate = docSlots[slotIndex][0].datetime.toDateString();

    try {
      const response = await fetch(
        `http://localhost:3002/api/user/${currentUser._id}/appointments`,
        {
          method: 'POST',
      ...
      if (response.ok) {
        ...
        fetchAppointments(currentUser._id); // Refresh context
      } else {
